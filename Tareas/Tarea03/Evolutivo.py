...
Algoritmo Evoulutivo para resolver  SCHWEFEL 
...
sympyf = 418.9829 * 2 - x1 * sin(sqrt(Abs(x1))) - x2 * sin(sqrt(Abs(x2)))
...
def seleccion_mas_clearing(
    ...
    total.sort(key=lambda x: (x[2], -x[3]))
...
def memetico(hijos_genotipos, step, iteraciones, npop, nvars,lb,ub):
...
def EA(f, lb, ub, pc, pm, nvars, npop, ngen, step, bl_iteraciones, radio):
    ...
    for i in range(ngen):
        ...
        hijos_genotipos = memetico(hijos_genotipos, step, bl_iteraciones, npop, nvars,lb,ub)
